    ...
    public class BookingController {
        ...
        public ResponseEntity<ApiResponse<?>> getUserBookings(
                @RequestParam(required = false) Long participantId,   // only used by ADMIN
                HttpServletRequest httpRequest) {
            try {
                String token = httpRequest.getHeader("Authorization").substring(7);
                Long requestingUserId = jwtUtil.extractUserId(token);

                List<UserBookingResponse> data = bookingService.getUserBookings(requestingUserId, participantId);
